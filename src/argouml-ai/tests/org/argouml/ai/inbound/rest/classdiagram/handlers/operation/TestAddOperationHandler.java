...
/**
 * Tests for {@link AddOperationHandler}.
 */
public class TestAddOperationHandler extends TestCase {
    ...
    public void testReturns201OnHappyPath() {
        ...
        ResponseEnvelope env = new AddOperationHandler(svc).handle(
        ...
        assertTrue("body must echo returnType void: " + env.body,
                env.body.contains("\"returnType\":\"void\""));
    }

    public void testReturns201WithoutReturnType() {
        svc.createClass(DIAGRAM, "Order", 10, 20, null, false);
        ResponseEnvelope env = new AddOperationHandler(svc).handle(
                ppFor("Order"), new HashMap<String, String>(),
                "{\"name\":\"flag\"}");
    ...
    public void testEmptyNameThrowsInvalidArgument() {
        svc.createClass(DIAGRAM, "Order", 10, 20, null, false);
        try {
            new AddOperationHandler(svc).handle(
                    ppFor("Order"), new HashMap<String, String>(),
    ...
    public void testMissingClassThrowsClassNotFound() {
        ...
        try {
            new AddOperationHandler(svc).handle(pathParams,
    ...
    public void testOperationIsAttachedToClass() {
        svc.createClass(DIAGRAM, "Order", 10, 20, null, false);
        ClassDiagramService.ClassView before =
                svc.getClass(DIAGRAM, "Order");
        int beforeCount = before.operations.size();
        new AddOperationHandler(svc).handle(
                ppFor("Order"), new HashMap<String, String>(),
                "{\"name\":\"save\",\"returnType\":\"void\","
                + "\"visibility\":\"public\"}");
        ClassDiagramService.ClassView after =
                svc.getClass(DIAGRAM, "Order");
        assertEquals("operation should be appended",
                beforeCount + 1, after.operations.size());
        boolean found = false;
        for (String enc : after.operations) {
            if (enc.startsWith("save")) {
                found = true;
                break;
            }
        }
        assertTrue("added operation should appear in ClassView: "
                + after.operations, found);
    }
}